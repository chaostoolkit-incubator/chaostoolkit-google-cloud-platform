...
def test_update_service(Credentials, dns_client):
    ...
    recordsets_patch.return_value.execute.return_value=fixtures.dns.response
    recordsets_svc.patch.return_value=recordsets_patch
    service.resourceRecordSets.return_value=recordsets_svc
    
    response=update_service( name=name,zone_name=zone_name,ip_address="10.2.0.6",secrets=fixtures.secrets,project=project_id)
